...
test("non-vendor JS files are reasonably sized (under 50KB each)", async () => {
  ...
  assert.deepEqual(oversized, [], "Non-vendor JS files exceeding 50KB");
});
...
test("coder.css is reasonably sized (under 100KB)", async () => {
  const fileStat = await stat(join(ROOT, "css", "coder.css"));
  const sizeKB = fileStat.size / 1024;
  assert.ok(sizeKB < 100, `coder.css is ${sizeKB.toFixed(1)}KB, exceeds 100KB`);
});
