...
在线聊天室保持静态站与动态状态分离：`/chat/` 只发布 Minnit 的受限嵌入容器，消息、在线状态和访客身份由 Minnit Chat 托管。第三方脚本与 iframe 来源只在聊天室页的 CSP 中精确放行，页面与请求继续绕过 PWA 缓存。仓库中的 `ChatRoom` 和 `ChatGate` Durable Objects 为停用的自托管替代实现，不参与当前页面运行。
