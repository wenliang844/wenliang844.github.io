...
### 1. [已修复] 分享 URL 依赖当前 `location.origin`，本地或预览环境可能分享错误域名

- 📌 问题/建议标题：分享链接应优先使用 canonical URL 或 `SITE.baseURL`
- 📍 位置：`js/share.js:14-35`、`src/templates/post.mjs:82-90`、`src/config.mjs:11-13`
- ✅ 修复状态：`share.js` 已优先读取当前页面 `<link rel="canonical">` 的 origin 作为相对分享路径基准；没有 canonical 或 canonical 异常时才回退到 `window.location.origin`。
- 🧪 回归测试：`tests/share-subscribe-feedback-deep.test.mjs` 覆盖本地预览地址下仍生成 `https://wenliang844.github.io/post/.../` 生产分享 URL；绝对分享 URL 仍保持原样。
...
### 2. [已修复] 评论脚本在文章页立即加载，缺少视口触发或用户同意层
...
- ✅ 修复状态：`giscus.js` 已在支持 `IntersectionObserver` 的浏览器中延迟到评论区接近视口（`rootMargin: "600px 0px"`）时再注入 `https://giscus.app/client.js`；不支持时保留立即加载 fallback。文章列表 switch 模式会先观察当前激活文章，评论区触发加载后再按 active slug 注入或切换 discussion。
- 🧪 回归测试：`tests/giscus-behavior.test.mjs` 覆盖单篇模式在 intersection 前不注入脚本、进入阈值后加载并断开 lazy observer；既有单篇、switch 模式、失败 fallback 和 pagehide 清理测试保持通过。
...
### 3. [已修复] Giscus 加载失败时评论区可能长期空白

- 📌 问题/建议标题：为第三方评论脚本增加 `onerror`、超时和可操作降级
- 📍 位置：`js/giscus.js:76-102`、`js/giscus.js:125-136`
- ✅ 修复状态：`buildScript()` 为 `https://giscus.app/client.js` 注入增加 `onerror` 和 12 秒超时兜底；失败时评论区渲染 `.comments-hint`，并通过 `CWLLogger.warn` 记录 `script-error` 或 `timeout` 原因。
- 🧪 回归测试：`tests/giscus-behavior.test.mjs` 覆盖外部脚本触发 `error` 后渲染“评论加载失败”提示并写入日志。
...
### 4. [已修复] Giscus 语言和主题不会跟随站内切换即时同步

- 📌 问题/建议标题：将站内语言/主题状态同步到 Giscus iframe
- 📍 位置：`js/giscus.js:15-23`、`js/giscus.js:76-95`、`js/coder.js:42-47`
- ✅ 修复状态：`coder.js` 每次应用主题时派发 `cwl:themechange`，携带 `mode` 和 `actualTheme`；`giscus.js` 初次注入脚本时从当前站点语言和 body 主题类设置 `data-lang` / `data-theme`，并在已加载 iframe 后监听 `cwl:langchange` 与 `cwl:themechange`，通过 giscus 官方 `postMessage({ giscus: { setConfig } })` 同步语言和主题。
- 🧪 回归测试：`tests/coder-deep.test.mjs` 覆盖主题事件细节；`tests/giscus-behavior.test.mjs` 覆盖英文/浅色初始脚本属性，以及语言/主题变化向 `.giscus-frame` 发送 `setConfig`。
...
### 6. [已修复] 微博分享窗口缺少弹窗拦截后的降级反馈

- 📌 问题/建议标题：`window.open` 返回 `null` 时应回退到普通链接或复制分享地址
- 📍 位置：`js/share.js:208-213`、`src/templates/post.mjs:86-89`
- ✅ 修复状态：微博分享现在先尝试打开分享页；成功时手动清空 `opener`，失败时把 `<a data-share="weibo">` 升级为 `href/target/rel` 完整外链，并尝试复制微博分享 URL；若复制也失败，则复用现有二维码兜底。
- 🧪 回归测试：`tests/share-subscribe-feedback-deep.test.mjs` 覆盖弹窗被拦截后 `<a>` 触发器升级为真实微博外链并复制分享 URL，以及按钮触发器在弹窗和复制都失败时显示二维码兜底。
...
2. 后续可谨慎评估 Giscus strict 映射，避免破坏既有 discussion 关联。
3. 长期可在评论区增加显式“加载评论”隐私模式，作为当前视口懒加载之上的更强用户同意层。
