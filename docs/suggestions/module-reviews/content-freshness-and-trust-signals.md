...
约束说明：本轮仅新增 `/docs/suggestions/module-reviews/content-freshness-and-trust-signals.md`，未修改源码、配置或测试。
...
1. 先让 sitemap 文章 `lastmod` 使用 `post.modified || post.date`，并补回归测试。
2. 在单篇文章 UI 中仅当 `modified !== date` 时显示“最后更新”。
3. 搜索索引加入 `modified`，搜索结果显示“发布/更新”日期。
4. 为旧项目文章设计 `status/reviewed/contextNote`，先从 2022/2023 项目补起。
5. 增加带 slug 的反馈入口，再评估是否公开源码链接。
6. 最后再决定是否做重大更新 RSS 或独立 updates feed。
