...
## 📌 MR-JS-04: `blog.js` 中 `buildItems()` 被调用两次

- **📍 位置**：`js/blog.js:174,317`
- **📝 当前状况**：`buildItems()` 在标签筛选初始化时调用一次（第 174 行），在文件末尾又调用一次（第 317 行）。第二次调用会覆盖第一次的结果，但内容相同。
- **⚠️ 影响程度**：低（冗余操作）
- **💡 建议方案**：删除第 317 行的重复调用，保留第 174 行的调用。
- **📊 预期收益**：消除冗余 DOM 查询
- **🔗 相关建议**：无
