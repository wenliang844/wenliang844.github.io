...
- 扩展组合验证 `node --test tests/build.test.mjs tests/build-extra.test.mjs tests/integration.test.mjs tests/format.test.mjs tests/performance.test.mjs tests/templates.test.mjs tests/templates-extended.test.mjs`：125/126 通过；唯一失败是当前工作区 `renderTrustPage()` 输出 `class="trust-stats timeline-stats"`，而 `tests/templates-extended.test.mjs` 仍精确匹配 `class="trust-stats"`，属于模板测试对 class 字面量的脆弱断言。
