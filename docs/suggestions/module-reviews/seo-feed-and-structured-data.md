...
- 扩展组合验证已修复此前 `renderTrustPage()` class 字面量断言脆弱问题；当前聚焦模板/构建/性能/工作流测试 `node --test tests/css.test.mjs tests/templates.test.mjs tests/templates-extended.test.mjs tests/performance.test.mjs tests/build.test.mjs tests/workflows.test.mjs` 111/111 通过，完整 `npm run test:coverage` 789/789 通过。
