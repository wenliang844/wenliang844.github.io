...
当前工作树已通过 `npm run lint:check`、`npm test` / 生产验证内部测试 792/792、`npm run test:coverage`、`npm run test:http-smoke`、`npm run test:browser-smoke`、`npm run validate:production`、`npm audit --registry=https://registry.npmjs.org --audit-level=moderate` 和 `git diff --check`。以下建议不修改源码，只记录视觉交互脚本中仍值得治理的边界。
