...
### 📌 F-13: AI 助手增加隐私模式和对话保留策略

- **📍 位置**：`js/assistant.js:34`, `js/assistant.js:220-243`, `js/assistant.js:1104-1108`, `js/assistant.js:1454-1478`
- **📝 当前状况描述**：AI 助手已经具备多轮 LLM 上下文和本地历史能力，但缺少用户可见的保存策略。对技术博客读者而言，常见输入可能包括 API 错误、配置片段、代码和业务上下文，适合提供更细粒度的隐私控制。
- **⚠️ 影响程度**：中
- **💡 建议方案**：
  ```text
  AI 助手设置：
  [x] 隐私模式：本轮不保存历史
  保留历史：会话结束即清除 / 7 天 / 30 天 / 永久
  [清除当前对话] [清除全部对话] [导出 JSON]
  ```
  隐私模式开启时仍允许当前页面内多轮上下文，但不写入 `cwl.assistant.conversations`。
- **📊 预期收益**：提升 AI 助手可信度，让它从“可用的聊天入口”演进为可长期使用的个人站点助手。
- **🔗 相关建议引用**：[S-14](security-audit.md#s-14-ai-助手对话和-llm-上下文长期留存在-localstorage), [UX-13](ux-improvements.md#ux-13-ai-助手默认模式与隐私文案需要重新对齐)
