...
- **✅ 修复状态**：AI 助手设置区已提供“隐私模式：不保存本轮对话”、历史保留期限（仅本次会话 / 7 天 / 30 天 / 永久）和“清空全部对话”；隐私模式与 session 保留不会写入 `cwl.assistant.conversations`，保留期限会清理过期对话。
- **🧪 验证**：`tests/assistant.test.mjs` 覆盖隐私模式不落盘、保留期限清理旧对话、session 临时会话、清空全部对话和英文文案；`tests/css.test.mjs` 覆盖隐私控件样式与移动端布局。
- **📝 原状况描述**：AI 助手已经具备多轮 LLM 上下文和本地历史能力，但缺少用户可见的保存策略。对技术博客读者而言，常见输入可能包括 API 错误、配置片段、代码和业务上下文，适合提供更细粒度的隐私控制。
- **⚠️ 影响程度**：中
- **💡 后续建议**：
  ```text
  AI 助手设置：
  [清除当前对话] [导出 JSON]
  ```
- **📊 实际收益**：提升 AI 助手可信度，让它从“可用的聊天入口”演进为可长期使用的个人站点助手。
- **🔗 相关建议引用**：[S-14](security-audit.md#s-14-已修复核心风险-ai-助手对话和-llm-上下文长期留存在-localstorage), [UX-13](ux-improvements.md#ux-13-ai-助手默认模式与隐私文案需要重新对齐)
