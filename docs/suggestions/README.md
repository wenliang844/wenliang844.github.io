...
| 质量门禁 | `npm run lint:check` 通过；`npm test` / `npm run test:coverage` 798/798 通过；`npm run test:http-smoke` 7/7 路由通过；`npm run test:browser-smoke` 通过 |
| 生产验证 | `npm run validate:production` 35/35 通过 |
| 依赖审计 | `npm audit --registry=https://registry.npmjs.org --audit-level=moderate` 0 漏洞 |
| 覆盖率 | 总体 lines 96.80%、branches 83.45%、functions 96.51%，通过阈值 |
...
| 当前质量基线 | `npm run lint:check` 通过；`npm test` 798/798 通过；`npm run test:coverage` line 96.80%、branch 83.45%、funcs 96.51%；HTTP smoke 7/7；生产验证 35/35 |
| 综合健康度 | 3.9 / 5（良好） |
