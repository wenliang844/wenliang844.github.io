...
`/chat/` 直接嵌入 Minnit Chat 托管房间，不依赖本站 Worker。构建时会为该页面单独放行 `https://minnit.chat` 脚本和 `https://organizations.minnit.chat` iframe；部署后应从目标网络验证这两个域名均可访问。仓库保留的 Durable Objects 聊天实现默认关闭，仅作为自托管替代方案，不参与当前页面运行。
